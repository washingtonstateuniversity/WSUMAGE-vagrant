...
Vagrant.configure("2") do |config|
    ...
    config.vm.define "web", primary: true do |web_config|
        ...
        if defined? VagrantPlugins::HostsUpdater
            web_config.hostsupdater.aliases = [ "web", "store.mage.dev", "events.store.mage.dev", "student.store.mage.dev", "general.store.mage.dev", "store.admin.mage.dev", "tech.store.mage.dev"  ]
        end
        if File.exist?("_BOXES/weblite_alt.box" )
            web_config.vm.box = "weblite_alt"
            web_config.vm.box_url = "_BOXES/weblite_alt.box"   ##saved state
        else
            if File.exist?("_BOXES/precise32.box" )
                web_config.vm.box = "precise32"
                web_config.vm.box_url = "_BOXES/precise32.box"   ##"http://images.wsu.edu/vmboxes/precise32.box"
            else
                web_config.vm.box = "precise32"
                web_config.vm.box_url = "http://images.wsu.edu/vmboxes/precise32.box"   ##"http://files.vagrantup.com/precise32.box"
            end
        end
